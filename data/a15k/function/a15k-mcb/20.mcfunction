...
## ---- roll: n = 2*x^2 + 15 ----
execute store result score @s a15k_probability run data get entity @s equipment.mainhand.components."minecraft:enchantments"."minecraft:projectile_protection"
scoreboard players operation @s a15k_probability *= @s a15k_probability
scoreboard players operation @s a15k_probability *= .c2 a15k_math
...
## on success
execute if score @s a15k_probability matches 0 run function a15k:a15k-mcb/21